	...
	if (e.currentStyle) {
		...
		fpGetCss = function(e, sName){
			try{
				...
				return (e.style[sName]||d.defaultView.getComputedStyle(e,null).getPropertyValue(sName.replace(/([A-Z])/g,"-$1").toLowerCase())).replace(",","");
			}catch(e){
				throw new Error(e.tagName||"document" + "는 css를 사용 할수 없습니다.");
			}
		}
 ...
 * 첫 번재. 메서드의 특성상 내부 문서가 완전히 로딩된 다음에 실행되어야 한다. 
 * 두 번째. 창이 내부 문서보다 큰 경우에는 내부 문서를 구할 수 없으므로, 반드시 창 크기를 내부 문서보다 작게 만든다.
 * 세 번째. 반드시 body에 사이즈가 있어야 한다.
 * 네 번째. html의 DOCTYPE이 Quirks일때 맥에서는 opera(10), 윈도우에서는 IE6+, opera(10), safari(4)에서 정상적으로 작동하지 않음.
 * @example
