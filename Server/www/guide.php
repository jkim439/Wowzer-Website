...
// 최근 게시물 로드
$result_tip_beginner = mysql_query("SELECT * FROM bbs_tip_beginner ORDER BY num DESC LIMIT 5", $dbconn);
$result_faq = mysql_query("SELECT * FROM bbs_faq ORDER BY num DESC LIMIT 5", $dbconn);
		...
		<tr>
			...
			<td	id="layout_right">
				<table id="layout_right_"	border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td	style="width:750px; height:37px; background-image:url('http://akeetes430.cdn2.cafe24.com/title_3_0.gif');"></td>
					</tr>
					<tr>
						<td	style="width:750px;"	bgcolor="white"	valign="top">
							...
							<p align="center"><font	face="Gulim"><span style="font-size:9pt;">
								...
								<table style="width:690px; height:200px;"	border="0" cellspacing="0" cellpadding="0">
									<tr>
										<td style="width:345px; height:50px; background-image:url('http://akeetes430.cdn2.cafe24.com/bbs_list_3_2.gif');"></td>
										<td style="width:345px; height:50px; background-image:url('http://akeetes430.cdn2.cafe24.com/bbs_list_3_3.gif');"></td>
									</tr>
									<tr>
										...
										<?
												while($list = mysql_fetch_array($result_tip_beginner)) {
											...
											<a href="#" target="_self" onclick="self.location.href='guide_2.php?mode=view&wno=<?=$list[wno]?>';" title="<?=$list[title]?>"><font color="#370909"><?if($new=="1"){?><strong><?=$title?></strong><?}else{?><?=$title?><?}?></font></a> <?if($new=="1"){?><img src="http://akeetes430.cdn2.cafe24.com/icon_new.gif" align="absmiddle" style="width:20px; height:13px;" alt="새 글"><?}?><br><br><?}?>
											</span></font></p>
										</td>
										...
										<?
												while($list = mysql_fetch_array($result_faq)) {
											...
											<a href="#" target="_self" onclick="self.location.href='guide_3.php?mode=view&wno=<?=$list[wno]?>';" title="<?=$list[title]?>"><font color="#370909"><?if($new=="1"){?><strong><?=$title?></strong><?}else{?><?=$title?><?}?></font></a> <?if($new=="1"){?><img src="http://akeetes430.cdn2.cafe24.com/icon_new.gif" align="absmiddle" style="width:20px; height:13px;" alt="새 글"><?}?><br><br><?}?>
											</span></font></p>
										</td>
									</tr>
								</table>
