...
$character_num_job = "character_".$vmember[character_code]."_job";
...
mysql_query("update member set $character_num_name='$vmember[character_name]' where no='$vmember[no]'", $dbconn);
mysql_query("update member set $character_num_job='$vmember[character_job]' where no='$vmember[no]'", $dbconn);
...
mysql_query("update admin_log set ok=ok+1 where no='1'",$dbconn);
       echo "<script>location.href='level_sub_1.php'</script>";
} else {
