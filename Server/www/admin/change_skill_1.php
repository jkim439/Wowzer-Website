...
$time=time();
if($mode=="off"){
$result = mysql_query("SELECT * FROM member WHERE level_state='1' and character_mode='3' ORDER BY level_time", $dbconn);
} else {
$result = mysql_query("SELECT * FROM member WHERE level_state='1' and character_mode='3' and level_time<=$time-172800 ORDER BY level_time", $dbconn);
}
?>

<center><?if($mode=="off"){?>전문기술 변경요청<br><a href="change_skill_1.php">[정상 모드]</a>&nbsp;[시간 제한 해제 모드]<?}else{?>전문기술 변경요청<br>[정상 모드]&nbsp;<a href="change_skill_1.php?mode=off">[시간 제한 해제 모드]</a><?}?>
<br><br>
...
?>
                    ...
                    <tr <?=$bgs?>>
                        ...
                        <td width='35' height='35'>
                            <p align='center'><b><font face='굴림' color='black'><span style='font-size:9pt;'><input type='button' name='ok' value="처리" style="font-family:'굴림'; font-size:'9pt';" onclick="location.href='change_skill_2.php?no=<?=$vmember[no]?>'"></span></font></b></p>
                        </td>
                    </tr><tr>
