...
if($key!= $keycode3){
  echo "<script>alert(' 보안 코드가 올바르지 않습니다. ');self.location.href='help_find_1_1.php';</script>"; exit;
}
...
if($email_length<5) {
  echo "<script>alert(' 이메일은 최소 5글자이여야 합니다. ');self.location.href='help_find_1_1.php';</script>"; exit;
}

// 이메일 체크
$email_result = mysql_query("select * from member where email='$email'");
$member_find = mysql_fetch_array($email_result);

// 탈퇴 회원
if($member_find[pw]=="0") {
	echo "<script>alert(' 삭제된 계정입니다. ');self.location.href='help_find_1_1.php';</script>"; exit;
}

if($member_find[email]) {
...
<br>
Eternal Soulmate의 아이디 찾기 메일입니다.<br>
<br>
<br>
고객센터에서 아이디 찾기 서비스를 이용하셨습니다.<br>
<br>
아래는 $member_find[name_nick]님의 아이디입니다.</b></font></span><font face='굴림'><span style='font-size:9pt;'></span></font></p>
        </td>
    </tr>
    <tr>
...
// 회원 가입 완료
echo "<script>alert(' 아이디를 입력하신 이메일로 전송하였습니다. ');self.location.href='home.php';</script>"; exit;
  
} else {
  echo "<script>alert(' 입력하신 이메일에 해당하는 계정이 없습니다. ');self.location.href='help_find_1_1.php';</script>"; exit;
}
