...
// 최근 게시물 로드
$result_1 = mysql_query("SELECT * FROM bbs_free ORDER BY time DESC LIMIT 5", $dbconn);
$result_2 = mysql_query("SELECT * FROM bbs_tip ORDER BY time DESC LIMIT 5", $dbconn);

// 긴급 공지시항 메시지
if($notice_emergency=="1" && $notice_emergency_alert=="1" && !$member[no]){
	echo "<script>alert('$notice_emergency_alert_text');</script>";
}
		...
		<tr>
			...
			<td	id="layout_right">
				<table id="layout_right_"	border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td	id="layout_right_notice" style="<?if($notice_emergency=="1" && $notice_emergency_img=="1"){?>background-image:url('http://akeetes430.cdn2.cafe24.com/notice/notice_emergency.jpg');<?}else{?>background-image:url('http://akeetes430.cdn2.cafe24.com/notice/notice.jpg');<?}?><?if($notice_img_url!="0"){?> cursor:pointer;<?}?>"<?if($notice_img_url!="0"){?>  onclick="self.location.href='<?=$notice_img_url?>';"<?}?> title="<?=$notice_img_title?>"></td>
					</tr>
				</table>
			</td>
		</tr>
