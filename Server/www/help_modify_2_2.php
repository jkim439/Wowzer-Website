...
if($key!= $keycode3){
  echo "<script>alert(' 보안 코드가 올바르지 않습니다. ');self.location.href='help_modify_2_1.php';</script>"; exit;
}
...
if($pw_check_length<4) {
  echo "<script>alert(' 비밀번호는 최소 4글자이여야 합니다. ');self.location.href='help_modify_2_1.php';</script>"; exit;
}
if($name_real_length<2) {
  echo "<script>alert(' 실명은 최소 2글자이여야 합니다. ');self.location.href='help_modify_2_1.php';</script>"; exit;
}
if($email_length<5) {
  echo "<script>alert(' 이메일은 최소 5글자이여야 합니다. ');self.location.href='help_modify_2_1.php';</script>"; exit;
}
...
if($email_check[email]) {
  echo "<script>alert(' 이미 사용 중인 이메일입니다. ');self.location.href='help_modify_2_1.php';</script>"; exit;
}
...
// 비밀번호와 실명 검사
if($member[pw]!=$pw) {
	echo "<script>alert(' 입력하신 정보가 올바르지 않습니다. ');self.location.href='help_modify_2_1.php';</script>"; exit;
}
...
// 이메일 인증키 생성
$email_key = md5(md5($key1).md5($pw).md5($email));
mysql_query("update member set login_state='4' where no='$member[no]'", $dbconn);
mysql_query("update member set email='$email' where no='$member[no]'", $dbconn);
mysql_query("update member set email_key='$email_key' where no='$member[no]'", $dbconn);
...
<br>
Eternal Soulmate의 이메일 변경에 관한 메일입니다.<br>
<br>
<br>
이 메일은 계정 정보 변경의 이메일 인증 메일입니다.<br>
<br>
...
session_destroy();
// 회원 가입 완료
echo "<script>alert(' 입력하신 이메일로 인증 메일을 보냈습니다. \\n\\n 이메일 인증을 하시면 변경이 완료됩니다. ');self.location.href='home.php';</script>"; exit;
